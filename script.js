...
window.onload = function () {
        ...
        $.getJSON("map.geojson", function (data) {
            geojsonLayer = L.geoJson(data, {
                ...
                filter: function (feature) {
                    ...
                    var isIMCOEnabled = $('#toggle-imco').is(':checked');
                    ...
                    var isIMCOValid = !isIMCOEnabled || feature.properties.IMCO == 1;

                    return (isSectorValid && isMunicipioValid && isPersonalValid && (filter === '' || isNameMatch) && isIMCOValid);
                }
            });
    ...
    $('#toggle-imco').on('change', function () {
        loadGeoJSON($('#search-box').val());
    });
};
...
function downloadCSV() {
    ...
    var csvData = [];
    var headers = ['NOMBRE', 'ACTIVIDAD', 'DOMICILIO', 'TELEFONO', 'SECTOR', 'IMCO'];
